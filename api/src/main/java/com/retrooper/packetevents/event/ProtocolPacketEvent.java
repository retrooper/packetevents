...
package com.retrooper.packetevents.event;
...
import com.retrooper.packetevents.event.type.CancellableEvent;
import com.retrooper.packetevents.event.type.PlayerEvent;
...
public abstract class ProtocolPacketEvent extends PacketEvent implements PlayerEvent, CancellableEvent {
    ...
    public ProtocolPacketEvent(PacketSide packetSide, ChannelAbstract channel, Object player, ByteBufAbstract byteBuf) {
        ...
        this.connectionState = PacketEvents.get().getPlayerManager().getConnectionState(channel);

        ClientVersion version = PacketEvents.get().getPlayerManager().getClientVersion(channel);
        if (version == null) {
            if (player != null) {
    ...
    public void setConnectionState(ConnectionState connectionState) {
        this.connectionState = connectionState;
        PacketEvents.get().getPlayerManager().changeConnectionState(channel, connectionState);
    }
