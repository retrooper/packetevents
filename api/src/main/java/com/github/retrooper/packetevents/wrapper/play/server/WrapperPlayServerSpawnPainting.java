...
public class WrapperPlayServerSpawnPainting extends PacketWrapper<WrapperPlayServerSpawnPainting> {
    ...
    public void write() {
        ...
        if (serverVersion.isNewerThanOrEquals(ServerVersion.V_1_13)) {
            ...
            PaintingType oldType = variantToPaintingType(v);
            if (oldType == null) {
                throw new IllegalStateException(
                        "Cannot write modern painting variant '" + v.getName() + "' on pre-1.13");
            }
            writeString(oldType.getTitle(), 13);
        }
        if (serverVersion.isNewerThanOrEquals(ServerVersion.V_1_8)) {
    ...
    public @Nullable PaintingVariant getVariant() {
        return variant;
    }
